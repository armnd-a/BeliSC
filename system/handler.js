   ...
   try {
      ...
      if (m.msg && m.msg.viewOnce) {
         ...
         if (/image/.test(m.mtype)) {
            client.sendImage(m.chat, media, m.text != '' ? m.text : '© NuxL-Bot', m)
         } else if (/video/.test(m.mtype)) {
            client.sendVideo(m.chat, media, m.text != '' ? m.text : '© NuxL-Bot', m)
         }
      }
      ...
      if (!/afk/.test(m.text) && m.isGroup) {
         if (users.afk > -1) {
            client.reply(m.chat, `Kamu kembali online, setelah afk selama ${Func.texted('bold', Func.toTime(new Date - users.afk))}\n\n• ${Func.texted('bold', 'Alasan')} : ${users.afkReason ? users.afkReason : '-'}`, m)
            users.afk = -1
            users.afkReason = ''
         }
      }
      if (!m.fromMe && !m.isGroup && !users.banned) {
         if (setting.groupmode && !isOwner && !isPrem) {
            if (!users.whitelist && m.text && (m.text.charAt(0) == myPrefix || Func.socmed(m.text)) && !/owner|premium/.test(m.text)) return client.fakeGroupLink2(m.chat, Func.texted('bold', `bot sedang dalam mode : khusus group*\n\nTidak bisa menggunakan bot di pesan pribadi\njika kamu ingin menggunakan di pesan pribadi\n*silahkan upgrade premium hanya 10rb*\nSilahkan ketik *.owner / .premium`), global.db.setting.header, m.sender, m)
         }
      ...
      if (typeof m.text != 'object' && m.text && m.text.length != 1 && (isPrefix = (myPrefix || '')[0])) {
         ...
         for (let name in global.client.commands) {
            ...
            if (typeof cmd.limit != 'undefined' && cmd.limit && !isOwner && global.db.users[m.sender].limit > 0) {
               global.db.users[m.sender].limit -= cmd.limit || 1
            }
            if (typeof cmd.limit != 'undefined' && cmd.limit && global.db.users[m.sender].limit < 1) {
