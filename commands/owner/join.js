exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!id.endsWith('g.us')) return client.reply(m.chat, Func.texted('bold', `maaf kak, aku tidak bisa bergabung`), m)
         let member = await (await client.groupMetadata(id)).participants.map(v => v.id)
         return client.reply(m.chat, `Bot telah berhasil bergabung`, m)
      } catch {
         return client.reply(m.chat, Func.texted('bold', `maaf kak, aku tidak bisa bergabung`), m)
      }
   },
   owner: true
}